 ...
 */
 ...
 void ProtocolGame::AddItem(NetworkMessage &msg, uint16_t id, uint8_t count)
 {
	 ...
	 else if (version >= 1200 && it.isContainer() && player->getOperatingSystem() <= CLIENTOS_NEW_MAC)
	 {
		 ...
		 msg.addByte(0x00); // boolean (is brand new)
	 } else if (it.showClientDuration) {
		 msg.add<uint32_t>(it.decayTime);
		 msg.addByte(0x00); // boolean (is brand new)
	 }
	 
	 // Support for protocol 13.10+ features
	 if (it.isPodium()) {
		 msg.add<uint16_t>(0); // looktype
		 msg.add<uint16_t>(0); // lookmount
		 msg.addByte(2);       // direction
		 msg.addByte(0x01);    // is visible (bool)
	 }
 ...
 void ProtocolGame::AddItem(NetworkMessage &msg, const Item *item)
 {
	 ...
	 if (version >= 1280 && it.showClientCharges) {
		 ...
		 msg.addByte(0); // boolean (is brand new)
 ...
 void ProtocolGame::onRecvFirstMessage(NetworkMessage &msg)
 {
	 ...
	 if (version >= 1200 && msg.getLength() - msg.getBufferPosition() == 141)
	 {
		 msg.skipBytes(13);
	 }
 ...
 void ProtocolGame::sendBlessStatus()
 {
	 ...
	 uint16_t flag = 0;
	 uint16_t pow2 = 2;
	 for (int i = 1; i <= maxClientBlessings; i++) {
		 if (player->hasBlessing(i)) {
			 if (i > 1)
				 blessCount++;
			 flag |= pow2;
 ...
 {
	 // TODO: Implement for protocol 13.10+ when store balance is needed
	 // For now, just send coin balance as fallback
	 sendCoinBalance();
 }
 ...
 {
	 NetworkMessage msg;
	 msg.addByte(version >= 1200 ? 0x83 : 0x85);
 ...
 {
	 if (version >= 1200 && creature->isHealthHidden())
	 {
		 return;
	 }
 ...
 void ProtocolGame::AddCreature(NetworkMessage &msg, const Creature *creature, bool known, uint32_t remove)
 {
	 ...
	 if (version >= 1200 && creatureType == CREATURETYPE_MONSTER)
	 {
		 const Creature *master = creature->getMaster();
		 if (master)
		 {
	 ...
	 if (version >= 1200 && creatureType == CREATURETYPE_PLAYER)
	 {
