...
bool ConfigManager::load()
{
	...
	if (!loaded) { //info that must be loaded one time (unless we reset the modules involved)
		...
		string[MAP_NAME] = getGlobalString(L, "mapName", "demonHelmet");
		string[MAP_AUTHOR] = getGlobalString(L, "mapAuthor", "demonHelmet");
		...
		string[MAP_CUSTOM_AUTHOR] = getGlobalString(L, "mapCustomAuthor", "demonHelmet");
		...
		string[MYSQL_DB] = getGlobalString(L, "mysqlDatabase", "database");
		string[MYSQL_SOCK] = getGlobalString(L, "mysqlSock", "");
		string[CLIENT_VERSION_STR] = getGlobalString(L, "clientVersionStr", "13.10");
		...
		integer[CLIENT_VERSION] = getGlobalNumber(L, "clientVersion", 1310);
	...
	integer[REWARD_BAG_DURATION] = getGlobalNumber(L, "rewardBagDuration", 7200);
